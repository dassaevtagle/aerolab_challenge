...
export class ProductsPaginationComponent implements OnInit {
  ...
  update() {
    ...
    const { orderedProducts, pagination, visibleProducts } = this.pureUpdate(
      this.products,
      this.pagination,
      filterBy,
      sortBy
  ...
  pureUpdate(
    ...
    const newPagination = this.paginateOrderedProducts(
      newOrderedProducts,
      pagination
    );
    const newVisibleProducts = this.setVisibleProducts(
      newOrderedProducts,
      newPagination
    );
    ...
    if (sortBy) {
      switch (sortBy) {
        ...
        case "highest":
          ...
          break;
        case "recent":
        default: //All products are already ordered by most recent
